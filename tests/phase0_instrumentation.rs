...
fn uri(name: &str) -> fluent_uri::Uri<String> {
    plingo::utils::Span::new(format!("test://{name}"), 0, 0).unwrap().uri
}
...
fn local_edit_reports_document_scoped_work_counters() {
    ...
    let report = ws
        .edit(replace_first_in(&u, &mut mirror, "1", "42"))
        .expect("edit commits");
    ...
    let parser = report.work().parser(&u.to_string()).expect("layout snapshot work");
...
fn equal_edit_is_cold_but_reports_validation() {
    ...
    let mut mirror = HEAD_EDIT_DOC.to_string();
    let report = ws.edit(replace_first_in(&u, &mut mirror, "\"head\":1", "\"head\":1")).unwrap();
    ...
    let source = report.work().source(&u.to_string()).expect("validation recorded");
...
fn repeated_runs_produce_identical_counters() {
    let run_once = || {
        ...
        ws.open(u.clone(), HEAD_EDIT_DOC).unwrap();
        let mut mirror = HEAD_EDIT_DOC.to_string();
        let r1 = ws.edit(replace_first_in(&u, &mut mirror, "1", "7")).unwrap();
        let r2 = ws.edit(replace_first_in(&u, &mut mirror, "7", "8")).unwrap();
        (
...
fn failing_command_rolls_back_roots_facts_and_publication_identity() {
    ...
    assert_eq!(before_projection, after_projection, "canonical projection unchanged after rollback");
    let after_tokens = ws
        .snapshot()
        .observe::<Tokens<JsonToken>>(u.to_string())
...
fn failed_command_isolates_documents_and_keeps_metrics_clean() {
    ...
    let b_work = report.work().parser(b.as_str()).cloned().unwrap_or_default();
